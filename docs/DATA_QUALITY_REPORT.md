...
Generated 2026-07-18 16:33 UTC
...
-> 2019 public transport is bus-only. THA25 added rail later, so 2019 rail is missing; `pt_total_complete = False` flags it. 
   Treat 2019 PT as a bus baseline.
-> 'Vehicle Population' is registered stock, not live traffic. Intensity uses the `Kilometres Travelled` measure (millions of km).
-> TEM22 -> TEM23 definitional break at 2022 is bridged but made explicit via `source_table` / `definition`.
